...
uint32_t mem_read_32(uint32_t address) {
  ...
  for (i = 0; i < MEM_NREGIONS; i++) {
    if (address >= MEM_REGIONS[i].start &&
      ...
      return (MEM_REGIONS[i].mem[offset + 3] << 0) |
...
*/
char *builtin_str[] = {"g",     "G",     "go", "r",    "R",    "run",
                       "mdump", "?",     "h",  "help", "q",    "Q",
                       "quit",  "rdump", "i",  "I",    "input"};

int (*builtin_func[])(char **) = {
    &go,         &go,    &go,        &run,       &run,        &run,
    &mdump,      &help,  &help,      &help,      &exit_shell, &exit_shell,
    &exit_shell, &rdump, &input_cmd, &input_cmd, &input_cmd};
